...
function tellbestmove(){
    if(gameover == "NO"){
        var gameoverbackup = gameover;
        virturn[0] = turn;
        if(turn == 0) virturn[1]=1;
        else virturn[1]=0; 
        var bestmove = getbestmove();
        if(bestmove == "draw"){
            bestmove = onlyonemove;
        }
        gameover = gameoverbackup;
        if(bestmove != "ilose"){
            setTimeout(() => {
                document.getElementById(bestmove+1).style.backgroundColor= "#68238a" ;
            }, 200);
            setTimeout(() => {
                document.getElementById(bestmove+1).style.backgroundColor= "#b03bea" ;
            }, 500);
        }else{
            document.getElementById("bestmove").innerText= "No Best Move :)";
            setTimeout(() => {
                document.getElementById("bestmove").innerText= "Get Best Move" ;
            }, 1000);
        }
    }   
}
